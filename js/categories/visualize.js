...
/**
 * Generate 3D isometric SVG cube puzzle data
 * @param {Function} [customRi] - Optional RNG function (min, max)
 * @param {number} d - Difficulty
 * @param {number} qn - Question number
 * @param {string} [idPrefix] - Unique ID prefix to prevent SVG defs collision
 * @returns {{svg: string, ans: number}}
 */
const generateLegoSvgData = function(customRi, d, qn, idPrefix) {
  var randFn = customRi || ri;
  var pfx = idPrefix || '';
  var useTwoPiles = qn >= 5;

  function makePile(gW, gD, mH) {
    var grid = []; var tot = 0;
    for (var r = 0; r < gD; r++) {
      grid[r] = [];
      for (var cc = 0; cc < gW; cc++) {
        var rowMax = Math.max(1, mH - r * 2);
        var rowMin = r === 0 ? 1 : 0;
        var h = randFn(rowMin, rowMax);
        grid[r][cc] = h;
        tot += h;
      }
    }
    ...
    for (var cc = 0; cc < gW; cc++) {
      for (var r = 1; r < gD; r++) {
        if (grid[r][cc] >= grid[r - 1][cc] && grid[r - 1][cc] > 0) {
          ...
          tot = 0;
          for (var rr = 0; rr < gD; rr++) for (var c2 = 0; c2 < gW; c2++) tot += grid[rr][c2];
        }
      }
    }
    if (tot === 0) { grid[0][0] = 1; tot = 1; }
  ...
  if (!useTwoPiles) {
    ...
    while (p1.tot > 28) {
      for (var r = 0; r < p1.gD && p1.tot > 28; r++) {
        for (var cc = 0; cc < p1.gW && p1.tot > 28; cc++) {
          if (p1.grid[r][cc] > 1) { p1.grid[r][cc]--; p1.tot--; }
        }
      }
    }
    piles.push(p1);
  } else {
    var gW1 = randFn(2, 3), gD1 = 2, mH1 = 2 + Math.min(d, 3);
    var gW2 = randFn(2, 3), gD2 = 2, mH2 = 2 + Math.min(d, 2);
    var p1 = makePile(gW1, gD1, mH1), p2 = makePile(gW2, gD2, mH2);
    while (p1.tot + p2.tot > 28) {
      if (p1.tot >= p2.tot) {
        for (var r = 0; r < p1.gD && p1.tot + p2.tot > 28; r++) {
          for (var cc = 0; cc < p1.gW && p1.tot + p2.tot > 28; cc++) {
            if (p1.grid[r][cc] > 1) { p1.grid[r][cc]--; p1.tot--; }
          }
        }
      } else {
        for (var r = 0; r < p2.gD && p1.tot + p2.tot > 28; r++) {
          for (var cc = 0; cc < p2.gW && p1.tot + p2.tot > 28; cc++) {
            if (p2.grid[r][cc] > 1) { p2.grid[r][cc]--; p2.tot--; }
          }
        }
      }
    }
  ...
  piles.forEach(function(p) {
    if (p.gW > maxGW) maxGW = p.gW;
    if (p.gD > maxGD) maxGD = p.gD;
    for (var r = 0; r < p.gD; r++) for (var cc = 0; cc < p.gW; cc++) {
      if (p.grid[r][cc] > maxH) maxH = p.grid[r][cc];
    }
  });
  ...
  function renderPile(pile, offX) {
    ...
    for (var r = 0; r < gD; r++) {
      for (var cc = 0; cc < gW; cc++) {
        var h = grid[r][cc]; var cl = pal[(r * gW + cc) % pal.length];
        for (var hh = 0; hh < h; hh++) {
          var bx = offX + (cc - r) * CW, by = oy + (cc + r) * CH - hh * CV;
          var gi = pfx + 'g' + offX + '_' + r + '_' + cc + '_' + hh;
          s += '<defs>';
  ...
  if (useTwoPiles) {
    ...
    svg = '<svg class="cube-svg" viewBox="0 0 ' + totalW + ' ' + svgH + '" style="width:100%;flex-shrink:1;max-height:100%;max-width:100%">';
    ...
    svg = '<svg class="cube-svg" viewBox="' + (leftEdge < 0 ? leftEdge - 5 : 0) + ' 0 ' + (totalW - Math.min(0, leftEdge) + 10) + ' ' + svgH + '" style="width:100%;flex-shrink:1;max-height:100%;max-width:100%">';
    svg += renderPile(p1, ox);
  }
  svg += '</svg>';

  return { svg: svg, ans: ans };
};

/** Generate visualize question */
const gVis = function(d) {
  var c = document.getElementById('gCard');
  legoIn = ''; legoKB = true;
  var data = generateLegoSvgData(ri, d, S.qn, '');
  legoAns = data.ans;

  c.innerHTML = '<div class="ins">Count ALL cubes (hidden too)</div>' +
    '<div style="flex:1;min-height:0;display:flex;align-items:center;justify-content:center;overflow:hidden;width:100%">' + data.svg + '</div>' +
